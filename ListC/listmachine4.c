...
void sweepPhase()
{
    ...
    while(i < HEAPSIZE)
    {
        ...
        if(Color(element_in_heap[0]) == White)
        {
            ...
            // Join any number of adjacent dead blocks into a single dead block
            word *next_free_block = &heap[i + Length(element_in_heap[0]) + 1];
            word *next_next_free_block = (word *) next_free_block[1];
            int totalLenght = Length(element_in_heap[0]);

            while(Color(next_free_block[0]) == White)
            {
                totalLenght += next_free_block[0];
                element_in_heap[1] = next_next_free_block[0];
                next_free_block = (word *) next_next_free_block[0];
                next_next_free_block = (word *) next_next_free_block[1];
            }

            element_in_heap[0] = mkheader(0, totalLenght, Blue);
